...
class NavigationEndpoint extends YTNode {
  ...
  constructor(data: any) {
    ...
    if (data?.browseEndpoint) {
      const configs = data?.browseEndpoint
    ...
    if (data?.watchEndpoint) {
      const configs = data?.watchEndpoint?.watchEndpointMusicSupportedConfigs
      ...
      this.watch = {
        ...
        supported_onesie_config: data?.watchEndpoint
    ...
    if (data?.createPlaylistEndpoint) {
      if (data?.createPlaylistEndpoint.createPlaylistDialog) {
        this.dialog = Parser.parseItem(
          data?.createPlaylistEndpoint.createPlaylistDialog,
  ...
  callTest(
    actions: Actions,
    args?: { [key: string]: any; parse?: boolean },
  ...
  async #call(actions: Actions, client?: string) {
    ...
    if (this.continuation) {
      switch (this.continuation.request) {
        case 'CONTINUATION_REQUEST_TYPE_BROWSE': {
          return await actions.browse(this.continuation.token, {
            is_ctoken: true,
          });
        }
    ...
    if (this.like) {
      ...
      const response = await actions.engage(this.metadata.api_url, {
  ...
  async call(
    ...
    client?: string,
    parse?: false,
  ): Promise<ActionsResponse | undefined>;
  async call(
    actions: Actions,
    client?: string,
    parse?: boolean,
  ): Promise<ParsedResponse | ActionsResponse | undefined> {
