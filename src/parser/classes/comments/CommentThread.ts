...
class CommentThread extends YTNode {
  ...
  async getReplies() {
    ...
    const continuation = this.#replies.key('contents').parsed().array().get({
    ...
    this.replies = response?.on_response_received_endpoints_memo?.getType(
      Comment,
    ).map((comment) => {
  ...
  async getContinuation() {
    ...
    const response = await this.#continuation.button?.item().key('endpoint')
      .nodeOfType(NavigationEndpoint).callTest(this.#actions, { parse: true });
