...
export class MusicResponsiveListItem extends YTNode {
  ...
  constructor(data: any) {
    super();
    this.#flex_columns = Parser.parseArray<MusicResponsiveListItemFlexColumn>(
      data.flexColumns,
      MusicResponsiveListItemFlexColumn,
    );
    this.#fixed_columns = Parser.parseArray<MusicResponsiveListItemFixedColumn>(
      data.fixedColumns,
    ...
    this.overlay = Parser.parseItem<MusicItemThumbnailOverlay>(
      data.overlay,
  ...
  #parseVideoOrSong() {
    const is_video = this.#flex_columns[1].key('title').instanceof(Text).runs
  ...
  #parseSong() {
    ...
    const duration_text =
      this.#flex_columns[1].key('title').instanceof(Text).runs?.find((run) =>
    ...
    const album =
      this.#flex_columns[1].key('title').instanceof(Text).runs?.find((run) =>
        Reflect.get(run, 'endpoint')?.browse?.id.startsWith('MPR')
      ) as TextRun ||
      this.#flex_columns[2]?.key('title').instanceof(Text).runs?.find((run) =>
        Reflect.get(run, 'endpoint')?.browse?.id.startsWith('MPR')
      ) as TextRun;
    if (album) {
      this.album = {
    ...
    const artists = this.#flex_columns[1].key('title').instanceof(Text).runs
      ?.filter((run) =>
        Reflect.get(run, 'endpoint')?.browse?.id.startsWith('UC')
      ) as TextRun[];
    if (artists) {
  ...
  #parseVideo() {
    this.id = this.#playlist_item_data.video_id;
    this.title = this.#flex_columns[0].key('title').instanceof(Text).toString();
    this.views = this.#flex_columns[1].key('title').instanceof(Text).runs?.find(
      (run) => run.text.match(/(.*?) views/),
    )?.text;

    const authors = this.#flex_columns[1].key('title').instanceof(Text).runs
      ?.filter((run) =>
        Reflect.get(run, 'endpoint')?.browse?.id.startsWith('UC')
      ) as TextRun[];
    if (authors) {
    ...
    const duration_text =
      ...
      this.#fixed_columns[0]?.key('title').instanceof(Text).runs?.find((run) =>
        (/^\d+$/).test(run.text.replace(/:/g, ''))
      )?.text;
    duration_text && (this.duration = {
      text: duration_text,
  ...
  #parseAlbum() {
    ...
    const author = this.#flex_columns[1].key('title').instanceof(Text).runs
      ?.find((run) =>
        Reflect.get(run, 'endpoint')?.browse?.id.startsWith('UC')
      ) as TextRun;
    author && (this.author = {
