...
class Library {
  ...
  async #fetchPage(browse_id: string, fetchArgs = {}) {
    const response = await this.#actions.browse(browse_id, {
      ...fetchArgs,
      client: 'YTMUSIC',
    });
   ...
   */
  async #fetchAndParseTabContents(
    browse_id: string,
    ...
    const sections =
      page.contents_memo.get('SectionList')?.[0].as(SectionList).contents
        .array() as Array<any> || [];
    const contents_section = sections.find((section) =>
      section.header?.type === 'ItemSectionTabbedHeader'
    );
    ...
    return new LibraryItemList(
      getItemsFromDataNode(data_node) || [],
      filter,
      continuation,
      page,
      this.#actions,
  ...
  async getPlaylists(args?: { sort_by?: SortBy }) {
    const data = await this.#fetchAndParseTabContents(
      this.#getBrowseId('playlists'),
      (item) => item.item_type === 'playlist',
    );
  ...
  async getAlbums(args?: { sort_by?: SortBy }) {
    const data = await this.#fetchAndParseTabContents(
      this.#getBrowseId('albums'),
      (item) => item.item_type === 'album',
    );
    const sort_by = args?.sort_by || null;
    return sort_by ? this.#applySortBy(data, sort_by) : data;
  }
  ...
  async getArtists(args?: { sort_by?: SortBy }) {
    const data = await this.#fetchAndParseTabContents(
      this.#getBrowseId('artists'),
      (item) => item.item_type === 'library_artist',
    );
    const sort_by = args?.sort_by || null;
    return sort_by ? this.#applySortBy(data, sort_by) : data;
  }
  ...
  async getSongs(args?: { sort_by?: SortBy | 'random' }) {
    ...
    const shuffle_endpoint = shuffle
      ...
      )?.endpoint as NavigationEndpoint
      : null;
    ...
    if (shuffle) {
      if (!shuffle_endpoint) {
        ...
        throw new InnertubeError(
          'Unable to obtain endpoint for sort_by value \'random\'',
        );
      }
  ...
  async #fetchAndParseShuffledSongs(endpoint: NavigationEndpoint) {
    ...
    const response = await this.#actions.next({
      ...payload,
      client: 'YTMUSIC',
    });
    const page = Parser.parseResponse(response.data);
    const playlist_panel = page.contents_memo.get('PlaylistPanel')?.[0].as(
      PlaylistPanel,
    );
  ...
  async getSubscriptions(args?: { sort_by?: SortBy }) {
    const data = await this.#fetchAndParseTabContents(
      this.#getBrowseId('subscriptions'),
    );
    const sort_by = args?.sort_by || null;
    return sort_by ? this.#applySortBy(data, sort_by) : data;
  }
  ...
  async #applySortBy(data: LibraryItemList, sort_by: SortBy) {
    ...
    const dropdownItem = page?.contents_memo.get('DropdownItem')?.find(
      (item) => item.as(DropdownItem).label === SORT_BY_TEXTS[sort_by],
    )?.as(DropdownItem);
    ...
    if (!dropdownItem?.endpoint?.browse) {
      ...
      throw new InnertubeError(
        `Unable to obtain browse endpoint for sort_by value '${sort_by}'`,
      );
    }
    ...
    return this.#fetchAndParseTabContents(
      dropdownItem.endpoint.browse.id,
      data.filter,
      fetchArgs,
    );
  }
  ...
  async getRecentActivity(args: { all: boolean }) {
    ...
    if (all) {
      ...
      const section_list = page.contents_memo.get('SectionList')?.[0].as(
        SectionList,
      );
    ...
    const contents_section = sections.find(
      (section) =>
        section.header?.type === 'MusicCarouselShelfBasicHeader' &&
        section.header?.title.toString() === 'Recent activity',
    );
...
abstract class LibraryResultsBase {
  ...
  async getContinuation() {
    ...
    switch (this.#continuation.type) {
      case 'next':
        responsePromise = this.#actions.next({
          ...payload,
          ctoken: this.#continuation.token,
          client: 'YTMUSIC',
        });
        break;
      default:
        responsePromise = this.#actions.browse(this.#continuation.token, {
          ...payload,
          is_ctoken: true,
          client: 'YTMUSIC',
        });
  ...
  abstract parseContinuationContents(
    page: ParsedResponse,
    from_continuation: Continuation,
  ): Promise<LibraryResultsBase>;
}
...
class LibraryItemList extends LibraryResultsBase {
  ...
  constructor(
    ...
    page: ParsedResponse,
    actions: Actions,
    overrides?: { sort_by: SortBy | 'random' | null },
  ) {
    ...
    this.sort_by = (overrides?.sort_by !== undefined)
      ? overrides.sort_by
      : this.#getSortBy();
  }

  async parseContinuationContents(
    page: ParsedResponse,
    from_continuation: Continuation,
  ) {
    const data = page.continuation_contents?.as(
      MusicShelfContinuation,
      GridContinuation,
      PlaylistPanelContinuation,
    );
    const continuation = data?.continuation
      ? { ...from_continuation, token: data?.continuation }
      : null;
    return new LibraryItemList(
      data?.contents || [],
      this.#filter,
  ...
  #getSortBy() {
    const selected =
      this.page?.contents_memo.get('DropdownItem')?.filter((item) =>
        item.as(DropdownItem).selected
      ) as DropdownItem[] || [];
