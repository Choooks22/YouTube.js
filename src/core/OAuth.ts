...
export class OAuth {
  ...
  async #getUserCode() {
    ...
    const response = await this.#session.http.fetch_function(
      new URL('/o/oauth2/device/code', Constants.URLS.YT_BASE),
      {
  ...
  #startPolling(device_code: string) {
    const poller = setInterval(async () => {
      ...
      try {
        const response = await this.#session.http.fetch_function(
          new URL('/o/oauth2/token', Constants.URLS.YT_BASE),
          {
            body: JSON.stringify(data),
            method: 'POST',
            headers: {
        ...
        const expiration_date = new Date(
          new Date().getTime() + response_data.expires_in * 1000,
        );
  ...
  async revokeCredentials() {
    ...
    return this.#session.http.fetch_function(
      new URL(
        `/o/oauth2/revoke?token=${
          encodeURIComponent(this.#credentials.access_token)
        }`,
  ...
  async #getClientIdentity() {
    const response = await this.#session.http.fetch_function(
      new URL('/tv', Constants.URLS.YT_BASE),
  ...
  get has_access_token_expired(): boolean {
    const timestamp = this.#credentials
      ? new Date(this.#credentials.expires).getTime()
      : -Infinity;
