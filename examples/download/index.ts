...
import { createWriteStream, existsSync, mkdirSync, readFileSync } from 'fs.ts';
import { streamToIterable } from 'youtubei.js/dist/src/utils/Utils.ts';

(async () => {
  const yt = await Innertube.create({ cache: new UniversalCache() });

  const search = await yt.music.search('No Copyright Background Music', {
    type: 'album',
  });
